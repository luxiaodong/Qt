...
void QPointTranslateDialog::clickOK()
{
    ...
    QPointTranslate pointTranslate;
    for(int i = 0; i < size; ++i)
    {
        if(ui->listWidget->item(i)->checkState() == Qt::Checked)
        {
            if(pointTranslate.translateFile(m_fileList.at(i)) == true)
            {
    ...
    QMessageBox::information(this,"PointTranslateDialog", pointTranslate.m_lastError);
}
